...
class GetAllTasks @Inject constructor(private val repository: TaskRepository) {
    ...
    operator fun invoke(taskOrder: TaskOrder = TaskOrder.Name(OrderType.Ascending)): Flow<List<Task>> {
            ...
            when (taskOrder.orderType) {
                is OrderType.Ascending -> {
                    when (taskOrder) {
                        ...
                        is TaskOrder.Date -> tasks.sortedBy { it.id }
                    }
                }
                is OrderType.Descending -> {
                    when (taskOrder) {
                        ...
                        is TaskOrder.Date -> tasks.sortedByDescending { it.id }
                    }
                }
            }
        }
